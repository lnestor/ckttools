def create_signal_comparator(moddef, inputs1, inputs2, pass_index):
    ...
    for i, input_pair in enumerate(zip(inputs1, inputs2)):
        output_name = "sig_comp_%i_%i" % (pass_index, i)
        instance_name = "SIG_COMP_XOR_%i_%i" % (pass_index, i)
        moddef.create_ilist("xnor", instance_name, output_name, input_pair)
        xnor_outputs[i] = output_name

    output_name = "sig_comp_and_%i" % pass_index
    moddef.create_ilist("and", "SIG_COMP_AND_%i" % pass_index, output_name, xnor_outputs)
    return output_name
