...
def get_key_patterns(key_gate_info, input_pattern, primary_inputs, original_bench, fault_filename, iteration):
    ...
    if len(key_pattern_dict) > 0:
        key_patterns = key_pattern_dict[key_gate_info["key_gate_output_net"]]
...
def get_propagation_events(key_gate_info, locked_filename, primary_inputs, pattern_generator, num_samples):
    ...
    for i, input_pattern in enumerate(pattern_generator.sample(num_samples)):
        key_patterns = get_key_patterns(key_gate_info, input_pattern, primary_inputs, original_bench, fault_filename, i)
